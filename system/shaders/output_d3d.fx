...
#endif
#if (defined(KODI_TONE_MAPPING_REINHARD_2) || defined(KODI_TONE_MAPPING_ACES) || defined(KODI_HLG_TO_PQ))
...
#endif
#if (defined(KODI_TONE_MAPPING_REINHARD) || defined(KODI_TONE_MAPPING_REINHARD_2))
float g_toneP1;
float3 g_coefsDst;
float g_luminance;
...
#endif
#if (defined(KODI_TONE_MAPPING_REINHARD_2) || defined(KODI_TONE_MAPPING_ACES))
...
float4 output4(float4 color, float2 uv)
{
...
#endif
#if defined(KODI_TONE_MAPPING_REINHARD_2)
  color.rgb = inversePQ(color.rgb);
  color.rgb *= (10000.0f / g_luminance) * (2.0f / g_toneP1);
  color.rgb = pow(color.rgb, 0.4f);
  float luma = dot(color.rgb, g_coefsDst) * 0.4f;
  color.rgb *= reinhard(luma) / luma;
#endif
#if defined(KODI_TONE_MAPPING_ACES)
  color.rgb = inversePQ(color.rgb);
  ...
  color.rgb *= (1.25f / g_toneP1);
  color.rgb = pow(color.rgb, 0.28f);
#endif
#if defined(KODI_HLG_TO_PQ)
