                                 ...
                                 std::string& shader)
{
  ...
  meta_prim = "Primaries (meta): ";
  meta_light = "HDR light (meta): ";
      ...
      rb->displayMetadata.max_luminance.num)
  {
    ...
    meta_light += StringUtils::Format("max ML: {:.0f}, min ML: {:.4f}", maxML, minML);

    if (rb->hasLightMetadata && rb->lightMetadata.MaxCLL)
      meta_light += StringUtils::Format(", max CLL: {}, max FALL: {}", rb->lightMetadata.MaxCLL,
