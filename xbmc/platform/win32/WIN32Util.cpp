...
#else
  std::wstring gdiDeviceName{CDisplayUtilWin32::GetCurrentDisplayName()};
  if (gdiDeviceName.empty())
    return HDR_STATUS::HDR_TOGGLE_FAILED;

  const auto identifier{CDisplayUtilWin32::GetDisplayTargetId(gdiDeviceName)};
  if (!identifier)
    return HDR_STATUS::HDR_TOGGLE_FAILED;

  status = CDisplayUtilWin32::GetDisplayHDRStatus(identifier.value());
  if (status == HDR_STATUS::HDR_UNSUPPORTED)
    return HDR_STATUS::HDR_TOGGLE_FAILED;

  const bool enable = (status == HDR_STATUS::HDR_OFF) ? true : false;

  CLog::LogF(LOGINFO, "Set Windows HDR {}.", enable ? "On" : "Off");

  status = CDisplayUtilWin32::SetDisplayHDRStatus(identifier.value(), enable);

  if (status == HDR_STATUS::HDR_TOGGLE_FAILED)
  {
    CLog::LogF(LOGERROR, "Set Windows HDR has failed.");
  }
  else if (modeDesc.RefreshRate.Denominator != 0) // Restores previous graphics mode (refresh rate)
  {
    const float fps = static_cast<float>(modeDesc.RefreshRate.Numerator) /
...
HDR_STATUS CWIN32Util::GetWindowsHDRStatus()
{
  ...
  {
    for (const auto& path : CDisplayUtilWin32::GetDisplayConfigPaths())
    {
      const DisplayConfigId identifier{path.targetInfo.adapterId, path.targetInfo.id};

      status = CDisplayUtilWin32::GetDisplayHDRStatus(identifier);
