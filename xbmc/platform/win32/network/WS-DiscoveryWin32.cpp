  ...
  if (list && address)
  {
    ...
    if (type == L"Computer")
    {
      ...
      if (it == m_serversIPs.end())
      {
        ...
        CLog::Log(LOGINFO, "[WS-Discovery]: IP '{}' inserted into servers list.", FromW(ip));
      }
    }
  }
...
HRESULT STDMETHODCALLTYPE CClientNotificationSink::Remove(IWSDiscoveredService* service)
{
  ...
  if (address)
  {
    ...
    if (it != m_serversIPs.end())
    {
      ...
      CLog::Log(LOGINFO, "[WS-Discovery]: IP '{}' removed from servers list.", FromW(ip));
    }
  }
...
HRESULT STDMETHODCALLTYPE CClientNotificationSink::SearchFailed(HRESULT hr, LPCWSTR tag)
{
  ...
  CLog::Log(LOGINFO, "[WS-Discovery]: Initial servers search has failed (none server found).");
...
HRESULT STDMETHODCALLTYPE CClientNotificationSink::SearchComplete(LPCWSTR tag)
{
  ...
  CLog::Log(LOGINFO,
            "[WS-Discovery]: Initial servers search has finished OK with {} server(s) found:",
            m_serversIPs.size());
...
bool CWSDiscoverySupport::Initialize()
{
  ...
  if (S_OK == WSDCreateDiscoveryProvider(nullptr, &m_provider))
  {
    ...
    if (S_OK == CreateClientNotificationSink(&m_sink))
    {
      if (S_OK == m_provider->Attach(m_sink))
      {
        ...
        {
          m_initialized = true;
          CLog::Log(LOGINFO, "[WS-Discovery]: Daemon initialized OK. Now are listening Hello and "
                             "Bye packets from LAN.");
          return true;
        }
      }
    }
  ...
  CLog::Log(LOGERROR, "[WS-Discovery]: Daemon initialized has failed.");
